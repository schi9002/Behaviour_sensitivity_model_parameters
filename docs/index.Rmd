...
```{r, echo=F, message=F, warning=F, error=F, fig.width=1*6*2.5, fig.height=1*3*2.5}
lsa.bmi.df.time.series.diff.derv %>% 
  filter(focus.title=="SUGAR Kj per gram" ) %>%
  filter(gender.output=='Male') %>%
  filter(bmi.output=="With obesity") %>%
  filter(as.numeric(age.groups.output)<=6) %>% 
  ungroup() %>% 
  filter(input.value==max(input.value) | input.value==min(input.value)) %>%
  select(year, run, age.groups.output,value, value.base.model) %>% 
  gather(., run.class, value, value:value.base.model) %>% 
  group_by(run, run.class) %>% 
  mutate(id.group= cur_group_id()) %>% 
  filter(id.group!=4) %>% 
  mutate(run.class=factor(id.group, levels=c(1,2,3), labels=c("Lower extreme", "Initial assumption", "Upper extreme"))) %>% 
  mutate(class=1) %>% 
  ungroup() %>% 
  rename("Age output"="age.groups.output") %>%
  mutate(class=factor(class, levels=c(1,2,3), labels=c("Outcome","Derivative","Cumulative absolute \n difference"))) %>% 
  ggplot()+
  geom_line(aes(x=year, y=value, color=run.class, linetype=run.class))+
  facet_grid(rows = vars(class),cols=vars(`Age output`), scales = "free")+
  scale_color_manual(values=c( "blue","black", "red"))+
  scale_linetype_manual(values=c(1,2,1))+
  scale_x_continuous(breaks = c(seq(2010, 2050, 20))) +
  scale_y_continuous(labels = scales::percent)+
  guides(colour = guide_legend("SUGAR Kj per gram"),
         linetype = guide_legend("SUGAR Kj per gram"))+
  theme_bw()+xlab("Year")+
  theme(legend.position = "bottom",
        axis.title.y = element_blank(),
        axis.title.x = element_blank())+
  ggtitle("Model behaviour", subtitle = "% Males with obesity, for extreme values of energy density of sugar (kJ/g)")+
  ...
  lsa.bmi.df.time.series.diff.derv %>% 
             ...
             ungroup() %>% 
             filter(input.value==max(input.value) | input.value==min(input.value)) %>%
             select(year, run, age.groups.output,value.der, value.der.base.model) %>% 
             gather(., run.class, value, value.der:value.der.base.model) %>% 
             group_by(run, run.class) %>% 
             mutate(id.group= cur_group_id()) %>% 
             filter(id.group!=4) %>% 
             mutate(run.class=factor(id.group, levels=c(1,2,3), labels=c("Lower extreme", "Initial assumption", "Upper extreme"))) %>% 
  ...
  theme(legend.position = "bottom",
        axis.title.y = element_blank())+
  # ggtitle("Model behaviour: changes in SUGAR Kj per gram", subtitle = "Males with obesity") + 
  ...
  #              ungroup() %>% 
  #              filter(input.value==max(input.value) | input.value==min(input.value)) %>%
  #              mutate(cum_sum_diff.der_value.base.model=NA) %>% 
  #              select(year, run, age.groups.output,cum_sum_diff.der_value, cum_sum_diff.der_value.base.model) %>% 
  #              gather(., run.class, value, cum_sum_diff.der_value:cum_sum_diff.der_value.base.model) %>% 
  #              group_by(run, run.class) %>% 
  #              mutate(id.group= cur_group_id()) %>% 
  #              filter(id.group!=4) %>% 
  #              mutate(run.class=factor(id.group, levels=c(1,2,3), labels=c("Lower extreme", "Initial assumption", "Upper extreme"))) %>% 
...
lsa.bmi.df.time.series.diff.derv %>% 
  ...
  scale_x_continuous(breaks = c(seq(2010, 2050, 20))) +
  scale_y_continuous(labels = scales::percent)+
  guides(colour = guide_legend("VIGOROUS PA METs"),
         linetype = guide_legend("VIGOROUS PA METs"))+
  theme_bw()+xlab("Year")+
  theme(legend.position = "bottom",
        axis.title.y = element_blank(),
        axis.title.x = element_blank())+
  ggtitle("Model behaviour: changes in VIGOROUS PA METs", subtitle = "Males with obesity")+
